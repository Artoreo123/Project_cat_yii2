 ...
 */
class LocationInputHelper extends InputWidget
{
    public $apiKey;
    public $width = "100%;";
    public $height = "200px";
...
let latLanDivider = "$this->latLanDivider";
console.log({position:position});
console.log({latLanDivider:latLanDivider});
function geoLocation() {
    let mapField = document.getElementById("map-field");
    mapField.innerHTML  = mapField.innerHTML+'<div id="block-map"></div>';
    if (navigator.geolocation) {
       navigator.geolocation.getCurrentPosition(initMap, positionDenied);
    } else {
        initMap();
    }
}
...
function initMap(positionMap = false) {
    if(positionMap){
       console.log({positionMap:positionMap.coords.latitude});
       console.log({positionMap:positionMap.coords.longitude});
       $('#registerform-lat').val(positionMap.coords.latitude);
       $('#registerform-long').val(positionMap.coords.longitude);
       position = positionMap.coords.latitude+','+positionMap.coords.longitude;
    }
    let roles = ['Farmer','Purchaser','Seller'];
    let role = $('#registerform-role').val();
    if(roles.includes(role)){
        $('#map-field').css('display','block');
    }
